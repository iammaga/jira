...
@section('content')
    ...
    <!-- Форма массового удаления, кнопка создания и фильтры -->
        ...
        <div class="flex flex-col sm:flex-row sm:justify-end sm:items-center mb-6 space-y-4 sm:space-y-0">
            <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 w-full sm:w-auto">
                ...
                <select name="project_id"
                        class="w-full sm:w-48 border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                <button type="submit"
                    ...
                    <span class="sm:hidden ml-2">Фильтровать</span>
                </button>
                <button type="submit"
                    ...
                    <span class="sm:hidden ml-2">Удалить</span>
                </button>
                <a href="{{ route('laratrust.roles.create') }}"
                   ...
                   title="Создать роль">
                    ...
                    <span class="sm:hidden ml-2">Создать</span>
                </a>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <th class="px-2 py-2 md:px-6 md:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                        <a href="{{ route('laratrust.roles.index', ['sort' => 'name', 'direction' => $direction === 'asc' ? 'desc' : 'asc']) }}"
                           class="hover:text-blue-600 flex items-center">
                            <i class="fas fa-heading mr-1"></i> Название {{ $sort === 'name' ? ($direction === 'asc' ? '↑' : '↓') : '' }}
                        </a>
                    </th>
                    <th class="px-2 py-2 md:px-6 md:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                        <i class="fas fa-project-diagram mr-1"></i> Проекты
                    </th>
                    <th class="px-2 py-2 md:px-6 md:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">
                        <i class="fas fa-lock mr-1"></i> Прав
                    </th>
                ...
                @forelse ($roles as $role)
                    <tr class="hover:bg-gray-100 transition duration-150">
                        ...
                        <td class="px-2 py-2 md:px-6 md:py-4 whitespace-nowrap text-gray-900 text-sm w-4/12 sm:w-6/12">{{ $role->display_name ?? $role->name }}</td>
                        <td class="px-2 py-2 md:px-6 md:py-4 whitespace-nowrap text-gray-700 text-sm hidden md:table-cell">{{ optional($role->projects)->pluck('name')->implode(', ') ?? 'Нет проектов' }}</td>
                        <td class="px-2 py-2 md:px-6 md:py-4 whitespace-nowrap text-gray-700 text-sm hidden md:table-cell">{{ $role->permissions_count }}</td>
                        <td class="px-2 py-2 md:px-6 md:py-4 whitespace-nowrap space-x-1 md:space-x-2 flex items-center w-32 md:w-auto">
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-2 py-2 md:px-6 md:py-4 text-center text-gray-500 text-sm">Ролей не найдено</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
